...
Events SHOULD be batched together and sent in intervals, the interval duration MUST be the `heartbeatInterval` from the `init` event response. 
Events that have not been sent when the session stops MUST be sent regardless of interval.
